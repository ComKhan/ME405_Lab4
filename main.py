...
def draw(listy, lastloc, plotting):
    ...
    for instr in listy:
        if instr[0] == 'IN ':
            ...
            plotting.drawing = 0
            interpolated_xy_points = interpolate(instrconv(instr), lastloc) # converting instruction into points and interpolation of that data
...
if __name__ == "__main__":
    ...
    solenoid = task_share.Share ('I', thread_protect = False, name = "Share 0")
    Theta_Values = task_share.Queue ('L', 25000, thread_protect = False, overwrite = False,
                           name = "Time Vals")
    X_Y_Values = task_share.Queue ('L', 25000, thread_protect = False, overwrite = False,
                           name = "ADC Vals")
